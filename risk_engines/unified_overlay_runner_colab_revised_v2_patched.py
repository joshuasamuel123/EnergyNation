...
# === BEGIN: flat merged export helper ========================================
def _export_flat_merged_old(ind_only_df, mpo_only_df, ind_mpo_df, out_path):
    """
    Merge three scenario outputs into a single flat CSV:
      - Keep common predecessor columns once (from 'Unique ID' through 'k_star_adj')
      - Remove 'indigenous_flag_overlay' and 'mpo_flag' if they appear
      - Append scenario outputs to the right in order: ind_only, mpo_only, ind_mpo
      - Ensure headers are unique via scenario suffixes
      - Keep EV_base / EV_disc_base only once, from ind_only (as canonical base)
    """
    import pandas as pd

    def _get_base_cols(df):
        cols = list(df.columns)
        # Find anchor for predecessor block end
        anchor_candidates = [c for c in cols if c.lower().strip().replace(" ", "_") in ("k_star_adj","k*adj","k_star_adj.1")]
        if anchor_candidates:
            anchor = anchor_candidates[0]
        else:
            # fallback to first k_star* column if exact match not found
            kstars = [c for c in cols if c.lower().startswith("k_star")]
            if not kstars:
                raise ValueError("Could not locate 'k_star_adj'/'k*adj' to anchor predecessor columns.")
            anchor = kstars[0]
        end_idx = cols.index(anchor)
        return cols[: end_idx + 1]

    # Drop unwanted flags if present
    def _drop_flags(df):
        return df.drop(columns=[c for c in df.columns if c.lower() in ("indigenous_flag_overlay","mpo_flag","scenario_key","scenario_label")], errors="ignore")

    ind_df = _drop_flags(ind_only_df).copy()
    mpo_df = _drop_flags(mpo_only_df).copy()
    comb_df = _drop_flags(ind_mpo_df).copy()

    base_cols = _get_base_cols(ind_df)

    # Base frame with predecessor columns
    base = ind_df[base_cols + ["Unique ID"]].copy()
    # Keep base EV columns once
    base_extra = [c for c in ["EV_base","EV_disc_base"] if c in ind_df.columns]
    base = base.merge(ind_df[["Unique ID"] + base_extra], on="Unique ID", how="left")

    def _scenario_slice(df, scenario_suffix):
        # Select runner-generated overlay columns
        candidate_overlay_cols = [
            "years_in_pipeline","blended_overlay","delta_blended",
            "EV_overlay","delta_EV","EV_disc_overlay","delta_EV_disc",
            "cross_over_to_favorite","cross_out_to_underdog"
        ]
        keep = [c for c in candidate_overlay_cols if c in df.columns]
        # DO NOT carry EV_base/EV_disc_base here to avoid duplication
        ren = {c: f"{c}{scenario_suffix}" for c in keep}
        return df[["Unique ID"] + keep].rename(columns=ren)

    ind_slice = _scenario_slice(ind_df, "_ind_only")
    mpo_slice = _scenario_slice(mpo_df, "_mpo_only")
    comb_slice = _scenario_slice(comb_df, "_ind_mpo")

    merged = base.merge(ind_slice, on="Unique ID", how="left") \
                 .merge(mpo_slice, on="Unique ID", how="left") \
                 .merge(comb_slice, on="Unique ID", how="left")

    merged.to_csv(out_path, index=False)
    return merged
# === END: flat merged export helper ==========================================

if __name__ == '__main__':
    main()

    # === Flat merged export (new) ===
    try:
        # Expect variables: df_ind_only, df_mpo_only, df_mpi_ind (or similar).
        # We attempt common names; adjust if the runner uses different ones.
        merged_out = os.path.join(output_dir, "unified_overlay_flat_merged.csv")
        _export_flat_merged_old(df_ind_only.copy(), df_mpo_only.copy(), df_mpi_ind.copy(), merged_out)
        print(f"[flat-merge] Wrote {merged_out}")
    except Exception as e:
        print("[flat-merge] Skipped: could not produce merged output ->", e)


def _export_flat_merged(ind_only_df, mpo_only_df, ind_mpo_df, out_path):
    import pandas as pd
    def _get_base_cols(df):
        cols = list(df.columns)
        anchor_candidates = [c for c in cols if c.lower().strip().replace(" ", "_") in ("k_star_adj","k*adj","k_star_adj.1")]
    ...
    comb_df = _drop_flags(ind_mpo_df.copy())
    base_cols = _get_base_cols(ind_df)
    base_cols_unique = ["Unique ID"] + [c for c in base_cols if c != "Unique ID"]
    base = ind_df[base_cols_unique].copy()
    base_extra = [c for c in ["EV_base","EV_disc_base"] if c in ind_df.columns]
    if base_extra:
        base = base.merge(ind_df[["Unique ID"] + base_extra], on="Unique ID", how="left")
    def _scenario_slice(df, scenario_suffix):
    ...
    mpo_slice = _scenario_slice(mpo_df, "_mpo_only")
    comb_slice = _scenario_slice(comb_df, "_ind_mpo")
    merged = base.merge(ind_slice, on="Unique ID", how="left")                  .merge(mpo_slice, on="Unique ID", how="left")                  .merge(comb_slice, on="Unique ID", how="left")
    merged.to_csv(out_path, index=False)
    return merged
